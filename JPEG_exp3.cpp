...
void SaveFrame(AVPicture *picture, int width, int height){
	FILE *pFile;
	char szFilename[32];

	// Open file
	sprintf_s(szFilename, "./Data/test.ppm");
...
pixel_chunk IDCT(pixel_chunk F) {
	if (F.pc.size() != 8 || F.pc[0].size() != 8) {
		printf("pixel chunk size is worng.\n");
		getchar();
	}
...
pixel_chunk quantify(pixel_chunk f, int type) {
	if (f.pc.size() != 8 || f.pc[0].size() != 8) {
		printf("pixel chunk size is worng.\n");
		getchar();
	}
...
pixel_chunk unquantify(pixel_chunk F, int type) {
	if (F.pc.size() != 8 || F.pc[0].size() != 8) {
		printf("pixel chunk size is worng.\n");
		getchar();
	}
...
jpeg_pic Encoder_jpeg(AVFrame *pFrame, size_t width, size_t height)
...
void Decoder_jpeg(jpeg_pic jpeg_p, size_t width, size_t height) {
	...
	*/
	...
	avpicture_alloc(picture, PIX_FMT_RGB24, width, height);
	...
	//save as jpeg
	SaveFrame(picture, width, height);
		...
		if (packet->stream_index == videoindex) {
			...
			if (got_picture) {
				if (pFrame->pict_type == AV_PICTURE_TYPE_I) {
					...
					Decoder_jpeg(jepg, pCodecCtx->width, pCodecCtx->height);
					break;
				}
			}
		}
